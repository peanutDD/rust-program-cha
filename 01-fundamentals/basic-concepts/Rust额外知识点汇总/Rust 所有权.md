...
![1762439312345](image/Rust所有权/1762439312345.png)

## 案例二

> 程序为什么报错，b 为 false，if 条件语句永远不执行啊？
...
fn main() {
    ...
    println!("{}", s); // 错误：尝试使用已被移动的值
}
```

编译错误信息明确指出： error[E0382]: borrow of moved value: s

### 为什么即使条件为false也会报错？

这是Rust所有权系统的一个重要特性：

1. 编译时路径分析 ：Rust编译器在编译阶段会分析 所有可能的代码路径 ，而不仅仅是那些在运行时可能执行的路径
2. 静态检查 ：即使 b 的值是 false ，编译器仍然必须考虑如果条件为 true 时会发生什么
3. 所有权检查的严格性 ：在第6行 if b { s2 = s; } 中，如果 b 为 true ， s 的所有权会转移给 s2 ，那么第8行就不能再使用 s

### 解决方案

有几种方法可以修复这个错误：

1. 使用克隆（clone） ：如果可以接受性能开销

```rust
if b {
    s2 = s.clone(); // 克隆创建新的所有权
}
println!("{}", s); // 现在安全了
```
...
3. 使用引用 ：如果不需要转移所有权
   ...
   println!("{}", s); // 安全，因为s没有被移动
   ```

### 总结

这个错误体现了Rust所有权系统的严谨性 - 即使是理论上可能执行但实际上永远不会执行的代码路径，编译器也会进行严格的所有权检查，确保在任何情况下都不会出现悬垂指针或数据竞争。这正是Rust能够在编译时保证内存安全的关键机制之一。


## 案例三


![1762443225716](image/Rust所有权/1762443225716.png)
